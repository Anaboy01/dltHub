...
const PersonalInfo = ({ initialData, updateData, onBack, onSubmit, isSubmitting }) => {
  ...
  const filteredCountries = countries.filter((country) => {
    ...
    return countryName.includes(dialSearch.toLowerCase()) || dialCode.includes(dialSearch.toLowerCase())
  })

  // Responsive heading font size
  const headingFontSize = isMobile ? "32px" : isTablet ? "48px" : "64px"

  return (
    <section className="flex flex-col gap-[40px] md:gap-[60px] lg:gap-[95px] w-full items-center relative justify-center pt-[40px] md:pt-[60px] lg:pt-[78px] pb-[40px] md:pb-[50px] lg:pb-[65px] select-none px-4 md:px-6 lg:px-0">
      <div className="flex flex-col items-center gap-[10px] md:gap-[15px]">
        <h1 className="text-[#FC7C13] font-semibold text-center" style={{ fontSize: headingFontSize }}>
          Personal Information
        </h1>
        <p className="text-[#F7FCFE] font-medium text-[16px] md:text-[18px] text-center">
          Let's get to know you better.
        </p>
      </div>

      <form className="flex flex-col w-full items-center gap-[30px] md:gap-[40px] lg:gap-[50px]" onSubmit={onSubmit}>
        <div className="w-full md:w-[80%] lg:w-[60%] grid grid-cols-1 md:grid-cols-2 gap-y-[25px] md:gap-x-[40px] lg:gap-x-[151px] items-center justify-center">
          {/* Full Name */}
          <div className="w-full flex flex-col gap-[10px] md:gap-[20px]">
            <label
              htmlFor="fullName"
              className="text-left text-[#F7FCFE] font-semibold text-[18px] md:text-[20px] lg:text-[22px] tracking-[2px]"
            >
              Full name
            </label>
            <input
          ...
          <div className="w-full flex flex-col gap-[10px] md:gap-[20px]">
            ...
            <select
              id="country"
              value={countries.find((c) => c.name.common === initialData.country)?.cca2 || ""}
          ...
          <div className="w-full flex flex-col gap-[10px] md:gap-[20px] relative">
            <label
              htmlFor="phoneNo"
              className="text-left text-[#F7FCFE] font-semibold text-[18px] md:text-[20px] lg:text-[22px] tracking-[2px]"
            >
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  {selectedPhoneCountry ? (
                    <img
                      src={selectedPhoneCountry.flags.png || "/placeholder.svg"}
                ...
                {phoneDropdownOpen && (
                  <div className="absolute z-10 w-[200px] max-h-60 overflow-y-auto bg-black border border-[#464646] mt-1 rounded-[10px]">
                    ...
                    {filteredCountries.map((country) => {
                      const telCode =
                        country.idd?.root && country.idd.suffixes?.[0] ? country.idd.root + country.idd.suffixes[0] : ""
                      return (
                        <button
                          key={country.cca2}
                          type="button"
        ...
        <div className="flex flex-col md:flex-row w-full md:w-[80%] lg:w-[60%] gap-4 md:gap-6">
          {/* Back button - Preserve original styling and animation for tablet/desktop */}
          {isMobile ? (
            <button
              type="button"
              onClick={onBack}
              className="bg-gray-500 text-white text-[16px] font-medium p-[10px] rounded-[10px] w-full"
            >
              <IoArrowBack className="mx-auto md:mx-0 inline mr-2" />
              <span>Back</span>
            </button>
          ) : (
            <motion.button
              type="button"
              onClick={onBack}
              className="bg-gray-500 absolute left-8 top-1/2 -translate-y-1/2 text-white font-medium rounded-[10px] md:p-[15px] md:text-[20px] lg:p-[20px] lg:text-[24px]"
              animate={{ y: [0, -10, 0] }}
          ...
          <button
            type="submit"
            className="bg-buttonOrange text-white text-[16px] font-medium p-[10px] w-full md:w-[80%] lg:w-[50%] rounded-[10px] md:mx-auto"
          >
